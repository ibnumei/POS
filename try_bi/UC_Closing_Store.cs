...
namespace try_bi
{
    public partial class UC_Closing_Store : UserControl
    {
        ...
        String id_trans, view_date, article_id, id_list, date, id_Cstore, epy_id, epy_name, date_close_store, date_close_store2;
        ...
        {
            String sql = "SELECT * FROM transaction WHERE ID_C_STORE='" + id_Cstore + "' AND (STATUS='1' or STATUS='2')";
            holding(sql);
        }
        //=====================================================================================
        ...
        private void t_search_trans_OnTextChange(object sender, EventArgs e)
        {
            if (t_search_trans.text == "")
            {
                ...
                //holding(date);
                date = mydate.ToString("yyyy-MM-dd");
                String query2 = "SELECT * FROM transaction WHERE ID_C_STORE='" + id_Cstore + "' AND (STATUS='1' or STATUS='2')";
                holding(query2);
            }
            else
            {
                //String query = t_search_trans.text;
                date = mydate.ToString("yyyy-MM-dd");
                String query2 = "SELECT * FROM transaction WHERE ID_C_STORE='" + id_Cstore + "' AND (STATUS='1' or STATUS='2') AND TRANSACTION_ID LIKE '%" + t_search_trans.text +"%'";
                holding(query2);
            }
        }
